...
function gameinfo() {
    ...
    if ($nbplayers == 1)
        $players = array(
            array('name' => $noms[0],'score' => 0,'screw' => 0,'keycode' => array(98,66)), // Si on fournit un keycode, cela veut dire que le joueur est contrôlable en local (sinon, on renvoie "0")
        );
    elseif ($nbplayers == 2)
        $players = array(
...
function resources() {
    ...
    if ($mode == 'DisOrDat') {
        ...
        $strings = json_decode($qhdr['strings'], true);
        $nbanswers = 2;
